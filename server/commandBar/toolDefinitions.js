...
const TOOLS = [
  ...
  // --- WRITE (confirmation-card flow) -----------------------------------
  {
    ...
    input_schema: {
      type: 'object',
      properties: {
        account_name: { type: 'string', description: 'Find-or-create — the card will note if a new account will be created. If a near-match existing account is found instead, the tool returns choices rather than proposing creation.' },
        ...
        confirm_new_account: { type: 'boolean', description: 'Only set true after Stuart explicitly confirms creating a new account despite a near-match choice the tool previously returned.' },
      },
  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        account_name: { type: 'string', description: 'Employer/primary account — find-or-create, omit if unlinked. If a near-match existing account is found instead, the tool returns choices rather than proposing creation.' },
        business_unit: { type: 'string', enum: ['ASC', 'Simply Seated'] },
        phone: { type: 'string', description: 'Normalise dictated digits first (see system prompt). Rejected if not a plausible AU phone length.' },
        mobile: { type: 'string', description: 'Normalise dictated digits first (see system prompt). Rejected if not a plausible AU phone length.' },
        email: { type: 'string' },
        title: { type: 'string' },
        confirm_new_account: { type: 'boolean', description: 'Only set true after Stuart explicitly confirms creating a new employer account despite a near-match choice the tool previously returned.' },
        event_account_name: { type: 'string', description: 'ONLY set when Stuart\'s command actually mentions an exhibition/event/conference context (e.g. "met her at AHRI NCE 2026"). Find-or-create, linked via a separate event relationship — does not replace account_name.' },
        confirm_new_event_account: { type: 'boolean', description: 'Only set true after Stuart explicitly confirms creating a new event account despite a near-match choice the tool previously returned.' },
      },
